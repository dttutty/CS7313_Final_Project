...
Dataset can be download from https://zenodo.org/record/7213796 and need to be put in \DG_data folder, the original datasets are saved as DG_data\\<dataset\>\\<dataset\>.csv.
...
- The first line denotes the dataset format.
...
The dataset edge-lists can be pre-processed by ```preprocess_data/preprocess_data.py```.

After pre-processing the dataset edge-list, there are three files that are used by the models:

- <ml_dataset>.csv: this file contains the timestamp edge-list.

- <ml_dataset>.npy: this file contains the edge features in the dense `npy` format that has the features in binary format.

- <ml_dataset_node>.npy: this file contains the node features in the dense `npy` format that contains the node features in binary format.
