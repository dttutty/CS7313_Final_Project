...
class SwiGLU(nn.Module):
    """
    x: [..., 2 * hidden_dim]
    前一半做 gate (SiLU)，后一半是 up_proj，然后逐元素相乘
    """
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, attention_dim: int, num_heads: int, dropout: float = 0.1):
        ...
        self.multi_head_attention = MultiheadAttention(
            ...
            num_heads=num_heads,
            dropout=dropout,
            batch_first=False,  # 如果你想用 (B, L, D) 直接喂，可以改成 True
        )
        ...
        # --- 这里是 SwiGLU FFN 部分 ---
        # 传统 FFN 中间维度一般是 4 * d_model，你也可以保持这个设置
        hidden_dim = 4 * attention_dim
        # 先升维到 2 * hidden_dim（因为要切成两半做 SwiGLU）
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        """
        # MultiheadAttention 默认输入: (seq_len, batch_size, dim)
        ...
        # 转回 (batch_size, num_patches, dim) 方便后面使用
        x = x.transpose(0, 1)  # (B, L, D)

        # --- FFN + SwiGLU 子层（pre-norm + 残差）---
        residual = x
